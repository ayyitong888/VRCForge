...
async function forceCloseLaunch(launch) {
  ...
  await runPowerShell(`
    ...
    function Assert-TrackedRootIdentity {
      ...
      $rootCim = Get-CimInstance Win32_Process -Filter "ProcessId = $rootProcessId" -ErrorAction SilentlyContinue
...
async function closePackagedApp(launch) {
  ...
  if (quit.accepted && cleared.ok) {
    return { quit, trackedPid: rootPid, identityCheck, cdpOwnership, graceful: true, forced: false, finalSnapshot: cleared.snapshot };
  }
  ...
  try {
    ...
    phase.visible = await waitForEval(
      ...
      `(() => {
        ...
        const centralRuntimeLedgerAbsent = !document.querySelector("[data-vrcforge-runtime-activity-panel]");
        ...
        return {
          ok: environmentPanelFound && todoPanelFound && todoInsideEnvironment && centralRuntimeLedgerAbsent &&
            questionVisible && secondOptionVisible && eighthOptionVisible && recommendedVisible &&
            explanationTitle && optionsAreScrollable && !questionInRightRail && progressInTodoPanel &&
