...
async function main() {
  ...
  if (!output.lifecycle.ok) {
    output.assertions.failures.push("packaged app explicit Quit was not accepted or left an owned process/port alive");
    if (snapshotHasResidue(afterQuit)) {
      ...
      output.assertions.failures.push("failure cleanup required forced termination of the exact probe-owned launch");
    }
  }
