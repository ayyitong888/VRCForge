import { Edit3, Eye, EyeOff, Folder, FolderPlus, Loader2, RefreshCw, Trash2, X } from "lucide-react";
...
export function ProjectPickerModal({
  ...
  editProjectPathDraft?: string;
  onEditProjectPathChange?: (value: string) => void;
  onStartEditProject?: (path: string) => void;
        ...
        <div className="app-scrollbar mt-4 min-h-0 flex-1 space-y-5 overflow-y-auto pr-1">
          <div>
            <div className="mb-2 flex items-center gap-2 text-xs font-medium text-muted-foreground">
              <span className="flex-1">{t("project.scannedProjects")}</span>
              <Button type="button" variant="ghost" className="h-7 px-2 text-xs" disabled={loadingProjects} onClick={onRefreshProjects}>
                {loadingProjects ? <Loader2 className="mr-1 h-3.5 w-3.5 animate-spin" /> : <RefreshCw className="mr-1 h-3.5 w-3.5" />}
                {t("project.refreshProjects")}
              </Button>
            </div>
            {projectListError ? <p className="mb-2 rounded-md border border-destructive/40 px-3 py-2 text-xs text-destructive">{projectListError}</p> : null}
            {projects.length > 0 ? (
              <div className="space-y-1">
                {projects.map((project) => {
                  ...
                  return (
                    <div key={key}>
                    <div className="group flex min-w-0 items-center gap-2 rounded-md px-2 py-1.5 transition-colors hover:bg-muted">
                      <button
                        type="button"
                      ...
                      {(
                        ...
                        <button
                          ...
                          aria-label={t("project.removeFromList")}
                          disabled={saving}
                        ...
                        <button
                          type="button"
                          title={t("project.editPath")}
                          aria-label={t("project.editPath")}
                          disabled={saving}
                          className="shrink-0 rounded p-1 text-muted-foreground opacity-0 transition-colors hover:bg-background hover:text-foreground group-hover:opacity-100"
                          onClick={() => onStartEditProject?.(project.path || "")}
                        >
                    ...
                    {editingProjectPath === (project.path || "") ? (
                      <div className="ml-8 flex gap-2 px-2 pb-2">
                        <input value={editProjectPathDraft} onChange={(event) => onEditProjectPathChange?.(event.target.value)} className="h-8 min-w-0 flex-1 rounded-md border border-border bg-background px-2 text-xs" aria-label={t("project.editPath")} />
                        <Button type="button" className="h-8 px-2 text-xs" disabled={saving || !editProjectPathDraft.trim()} onClick={onSaveEditProject}>{t("common.save")}</Button>
                        <Button type="button" variant="ghost" className="h-8 px-2 text-xs" disabled={saving} onClick={onCancelEditProject}>{t("common.cancel")}</Button>
                      </div>
                    ) : null}
                    </div>
                  );
              ...
              <p className="rounded-md border border-dashed border-border px-3 py-2 text-xs text-muted-foreground">{loadingProjects ? t("project.scanningProjects") : projectListError ? t("project.projectListUnavailable") : discoveryPending ? t("project.scanNotStarted") : t("project.noProjectsHint")}</p>
            )}
          </div>
