...
async def broadcast_background_goal_state(_state: dict[str, Any]) -> None:
    await EVENT_BUS.broadcast("agentGoals", AGENT_GATEWAY.goal.list_agent_goals())
    await EVENT_BUS.broadcast("agentGoalBackground", AGENT_GATEWAY.goal.agent_goal_background_state())


BACKGROUND_GOAL_COORDINATOR = BackgroundGoalDeliveryCoordinator(
    goal=AGENT_GATEWAY.goal,
    approval=AGENT_GATEWAY.goal,
...
async def drain_timed_out_goal_wake(worker: asyncio.Task[dict[str, Any]]) -> None:
    ...
    try:
        state = await asyncio.to_thread(
            AGENT_GATEWAY.goal.defer_agent_goal_delivery_wake_timeout,
            delivery_id,
        )
    ...
    if BACKEND_OWNER_LEASE.owned:
        ...
        try:
            ...
            await asyncio.to_thread(
                AGENT_GATEWAY.goal.reconcile_agent_goal_watchdogs,
...
def app_agent_goals(limit: int = 50, sessionId: str = "", projectRoot: str = "") -> dict[str, Any]:
    return AGENT_GATEWAY.goal.list_agent_goals(limit=limit, session_id=sessionId, project_root=projectRoot)


@app.get("/api/app/agent/goals/due")
def app_due_agent_goals(limit: int = 20, sessionId: str = "", projectRoot: str = "") -> dict[str, Any]:
    return AGENT_GATEWAY.goal.list_due_agent_goals(limit=limit, session_id=sessionId, project_root=projectRoot)
...
async def app_create_agent_goal(request: AgentGoalCreateRequest) -> dict[str, Any]:
    ...
    try:
        payload = AGENT_GATEWAY.goal.create_agent_goal(params)
    except AgentGoalServiceError as exc:
        raise HTTPException(status_code=exc.status_code, detail=str(exc)) from exc
    await EVENT_BUS.broadcast("agentGoals", AGENT_GATEWAY.goal.list_agent_goals(limit=30, session_id=request.session_id or ""))
    return payload
...
async def app_wake_agent_goal(goal_id: str, request: AgentGoalWakeRequest) -> dict[str, Any]:
    worker = asyncio.create_task(
        asyncio.to_thread(
            AGENT_GATEWAY.goal.wake_agent_goal,
            goal_id,
            {
                "sessionId": request.session_id,
                "chatId": request.chat_id,
...
async def app_bind_agent_goal_owner(goal_id: str, request: AgentGoalOwnerBindRequest) -> dict[str, Any]:
    try:
        payload = AGENT_GATEWAY.goal.bind_agent_goal_owner(
            goal_id,
            {
                "sessionId": request.session_id,
                "chatId": request.chat_id,
...
def app_recoverable_agent_goal_deliveries(limit: int = 20, chatId: str = "") -> dict[str, Any]:
    return AGENT_GATEWAY.goal.list_recoverable_agent_goal_deliveries(limit=limit, chat_id=chatId)


@app.get("/api/app/agent/goals/background")
def app_agent_goal_background_state(chatId: str = "") -> dict[str, Any]:
    return AGENT_GATEWAY.goal.agent_goal_background_state(chat_id=chatId)
...
async def app_acknowledge_agent_goal_background_state(
    ...
    try:
        payload = AGENT_GATEWAY.goal.acknowledge_agent_goal_background_state(
...
async def app_materialize_agent_goal_delivery(
    ...
    try:
        payload = AGENT_GATEWAY.goal.materialize_agent_goal_delivery(
            delivery_id,
...
async def app_defer_agent_goal_delivery(
    ...
    try:
        payload = AGENT_GATEWAY.goal.defer_agent_goal_delivery_handoff(
            delivery_id,
...
async def app_update_agent_goal(goal_id: str, request: AgentGoalUpdateRequest) -> dict[str, Any]:
    ...
    if "wake_every_minutes" in request.model_fields_set:
        params["wakeEveryMinutes"] = request.wake_every_minutes
    try:
        payload = AGENT_GATEWAY.goal.update_agent_goal(goal_id, params)
    except AgentGoalServiceError as exc:
        raise HTTPException(status_code=exc.status_code, detail=str(exc)) from exc
    await EVENT_BUS.broadcast("agentGoals", AGENT_GATEWAY.goal.list_agent_goals(limit=30, session_id=request.session_id or ""))
    return payload
...
MEMORY_REVIEW: MemoryReviewComposition = build_memory_review_composition(
    MemoryReviewCompositionPorts(
        ...
        adapter=MemoryReviewDashboardAdapter(
            ...
            list_tasks=lambda: SUB_AGENT_COLLABORATION.list_tasks(
...
def app_list_sub_agents(includeEvents: bool = False, limit: int = 50) -> dict[str, Any]:
    return SUB_AGENT_COLLABORATION.list_tasks(include_events=includeEvents, limit=limit)
...
async def app_create_sub_agent(request: SubAgentCreateRequest) -> dict[str, Any]:
    ...
    try:
        payload = SUB_AGENT_COLLABORATION.create_task(
            role=request.role,
            task=request.task,
        ...
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    await EVENT_BUS.broadcast("subAgentTasks", SUB_AGENT_COLLABORATION.list_tasks())
    return payload


@app.get("/api/app/sub-agents/{task_id}")
def app_get_sub_agent(task_id: str) -> dict[str, Any]:
    payload = SUB_AGENT_COLLABORATION.get_task(task_id, include_events=True)
    if not payload.get("ok"):
...
async def app_cancel_sub_agent(task_id: str) -> dict[str, Any]:
    payload = SUB_AGENT_COLLABORATION.cancel_task(task_id)
    if not payload.get("ok"):
        raise HTTPException(status_code=404, detail=payload.get("error") or "Sub-agent task was not found.")
    await EVENT_BUS.broadcast("subAgentTasks", SUB_AGENT_COLLABORATION.list_tasks())
    return payload
...
async def app_retry_sub_agent(task_id: str) -> dict[str, Any]:
    payload = SUB_AGENT_COLLABORATION.retry_task(task_id)
    if not payload.get("ok"):
        raise HTTPException(status_code=404, detail=payload.get("error") or "Sub-agent task was not found.")
    await EVENT_BUS.broadcast("subAgentTasks", SUB_AGENT_COLLABORATION.list_tasks())
    return payload
...
async def app_merge_sub_agent(task_id: str, request: SubAgentMergeRequest) -> dict[str, Any]:
    payload = SUB_AGENT_COLLABORATION.merge_task(
        task_id,
...
async def app_acknowledge_sub_agent_handoff(task_id: str, request: SubAgentHandoffAckRequest) -> dict[str, Any]:
    payload = SUB_AGENT_COLLABORATION.acknowledge_handoff(
        task_id,
        expected_revision=request.expected_revision,
    )
    if not payload.get("ok"):
...
def _session_store_targets(context: dict[str, Any]) -> list[SessionStoreTarget]:
    subagent_targets = SUB_AGENT_COLLABORATION.maintenance_targets()
    targets = [
        SessionStoreTarget(
            "session.chat.app",
    ...
    for result_path in sorted(AGENT_GATEWAY.goal.result_dir.glob("*.json")):
        ...
        targets.append(
            SessionStoreTarget(
                ...
                guard_root=AGENT_GATEWAY.goal.result_dir,
            )
        )
...
def _repair_session_storage_doctor(context: dict[str, Any], _mode: str, phases: PhaseLog) -> dict[str, Any]:
    ...
    for target in _session_store_targets(context):
        ...
        if target.store_id.startswith("session.subagent"):
            with SUB_AGENT_COLLABORATION.maintenance_lock():
...
async def background_goal_monitor_loop() -> None:
    while True:
        try:
            payload = await asyncio.to_thread(AGENT_GATEWAY.goal.reconcile_agent_goal_watchdogs)
