...
The current published stable package is `1.8.5` (`v1.8.5`). Its manifest binds the desktop, backend and paired Core package to the release source commit. The v1.7.10 tag and Release remain
...
the `v1.8.5` package. The public repo must not contain encryption
...
powershell -NoProfile -ExecutionPolicy Bypass -File packaging\build_release.ps1 `
  -Version 1.8.5 `
  -PayloadDownloadUrl https://github.com/ayyitong888/VRCForge/releases/download/v1.8.5/VRCForge_Windows_x64_1.8.5.zip `
...
pointers when they exist in the checkout. For the current `1.8.5` target release,
