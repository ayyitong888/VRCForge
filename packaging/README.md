...
MCP Core, the fixed 78-tool registry, Editor lifecycle bootstrap, and the
...
The current source and latest published stable package is `1.7.8` (`v1.7.8`).
The release moves
the tag, manifest, source, and asset evidence together; the v1.7.7 tag and
Release page remain available and unchanged. It also supersedes
...
the `v1.7.8` package. The public repo must not contain encryption
...
powershell -NoProfile -ExecutionPolicy Bypass -File packaging\build_release.ps1 `
  -Version 1.7.8 `
  -PayloadDownloadUrl https://github.com/ayyitong888/VRCForge/releases/download/v1.7.8/VRCForge_Windows_x64_1.7.8.zip `
...
`2026-07-28`, all 78 VRCForge tools, automatic App connection to that exact
...
pointers when they exist in the checkout. For the current `1.7.8` target release,
