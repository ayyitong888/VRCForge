...
`1.3.4` is the current target stable package; `1.3.1` remains the latest
published stable package until `v1.3.4` is published. The Avatar Encryption /
...
powershell -NoProfile -ExecutionPolicy Bypass -File packaging\build_release.ps1 `
  -Version 1.3.4 `
  -PayloadDownloadUrl https://github.com/ayyitong888/VRCForge/releases/download/v1.3.4/VRCForge_Windows_x64_1.3.4.zip `
...
```powershell
python scripts\smoke_stable_readiness_gate.py --version 1.3.4 --latest-stable 1.3.1
```
...
pointers when they exist in the checkout. For the current `1.3.4` target stable line,
