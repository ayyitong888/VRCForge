...
The current source / target package is `1.7.9` (`v1.7.9`). `1.7.8` remains the latest published stable package
until formal publication. The release moves the tag, manifest, source, and
asset evidence together; the v1.7.8 tag and Release page remain available and
...
the `v1.7.9` package. The public repo must not contain encryption
...
powershell -NoProfile -ExecutionPolicy Bypass -File packaging\build_release.ps1 `
  -Version 1.7.9 `
  -PayloadDownloadUrl https://github.com/ayyitong888/VRCForge/releases/download/v1.7.9/VRCForge_Windows_x64_1.7.9.zip `
...
pointers when they exist in the checkout. For the current `1.7.9` target release,
