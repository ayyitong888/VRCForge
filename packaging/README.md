...
MCP Core, the fixed 97-tool registry, Editor lifecycle bootstrap, and the
...
The current source / target package is `1.8.6` (`v1.8.6`); `1.8.5` remains the latest published stable package. Its manifest binds the desktop, backend and paired Core package to the release source commit. The v1.7.10 tag and Release remain
...
the `v1.8.6` package. The public repo must not contain encryption
...
powershell -NoProfile -ExecutionPolicy Bypass -File packaging\build_release.ps1 `
  -Version 1.8.6 `
  -PayloadDownloadUrl https://github.com/ayyitong888/VRCForge/releases/download/v1.8.6/VRCForge_Windows_x64_1.8.6.zip `
...
`2026-07-28`, all 82 VRCForge tools, automatic App connection to that exact
...
pointers when they exist in the checkout. For the current `1.8.6` target release,
