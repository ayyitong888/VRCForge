...
| Face/shader adjustment checkpoint timeline | Separate high-frequency face and shader checkpoint index with manual create/read/update/delete, overwrite, A/B slot selection, preview, and apply through restore approval | 1.0.1 source line; applies remain approval/checkpoint/rollback gated |
...
Gateway writes and legacy desktop REST write endpoints create a pre-write checkpoint after saving open Unity scenes and dirty assets. Git worktrees use git-backed checkpoints; other projects use a local compressed baseline. Restore previews and applies only the changed, added, or deleted files, then reloads restored scenes and refreshes Unity assets. The desktop Checkpoints view lists, previews, and requests restore through the same approval path as other writes. Face tuning and shader tuning writes are also indexed into a separate Adjustment Timeline, where users can manually create, rename, delete, overwrite, select A/B comparison slots, preview, and apply an adjustment checkpoint through the same restore approval chain. Direct raw Unity MCP calls made outside VRCForge cannot be intercepted; use the supervised gateway or desktop write paths when rollback is required.
