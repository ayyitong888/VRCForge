...
MATERIAL_SOURCE = (ROOT / "Assets/VRCForge/Editor/MaterialShaderTool.cs").read_text(encoding="utf-8")


def test_single_material_and_texture_paths_use_targeted_save_only():
    shader_path = MATERIAL_SOURCE.index('public static object HandleCommand(JObject @params)')
    texture_path = MATERIAL_SOURCE.index('public static object HandleCommand(JObject @params)', shader_path + 1)
    shader_lane = MATERIAL_SOURCE[shader_path:texture_path]
    texture_lane = MATERIAL_SOURCE[texture_path:]
    assert shader_lane.count("AssetDatabase.SaveAssetIfDirty(target.material);") == 1
    assert "AssetDatabase.SaveAssets();" not in shader_lane
    assert texture_lane.count("AssetDatabase.SaveAssetIfDirty(material);") == 1
    assert "AssetDatabase.SaveAssets();" not in texture_lane
