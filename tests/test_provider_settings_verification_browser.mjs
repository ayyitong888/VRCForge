...
const bundle = await build({ stdin: { resolveDir: process.cwd(), loader: "tsx", contents: `
 ...
 function Fixture(){const [key,setKey]=useState(""); const [model,setModel]=useState("m"); const [passed,setPassed]=useState(false);
  const hook=useProviderSettings({endpoint:location.origin,runtimeConnected:true,apiConfig:{provider:"custom",base_url:"",model:"m",api_type:"auto",configured:true,apiKeyRequired:true,apiKeyPresent:true},startRuntime:async()=>location.origin,refresh:async()=>{},setError:()=>{}});
  useEffect(()=>setPassed(hook.providerTestPassed),[hook.providerTestPassed]);
  return <main><button data-testid="test" onClick={()=>void hook.runProviderTest("text")}>test</button><button data-testid="structured" onClick={()=>void hook.runProviderTest("structured")}>structured</button><input data-testid="key" value={key} onChange={e=>{setKey(e.target.value);hook.setApiKey(e.target.value)}}/><input data-testid="model" value={model} onChange={e=>{setModel(e.target.value);hook.setApiModel(e.target.value)}}/><div data-testid="passed">{String(passed)}</div><div data-testid="ready">{String(hook.providerReadyForOnboarding)}</div><div data-testid="testing">{hook.testingProvider}</div></main>;
...
let browser; try { browser=await chromium.launch({headless:true,channel:process.env.VRCFORGE_BROWSER_CHANNEL||"msedge"}); const page=await browser.newPage();
 ...
 assert.equal(await page.getByTestId("ready").innerText(),"false", "tested but unsaved key cannot finish onboarding");
 await page.getByTestId("key").fill("");
 await page.getByTestId("test").click(); await wait(()=>pending.length===1); reply({ok:true,status:"ok",capability:"text",message:"saved config"});
 await wait(async()=> (await page.getByTestId("ready").innerText()) === "true");
