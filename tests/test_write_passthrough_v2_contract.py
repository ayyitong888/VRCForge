...
def test_toggle_scene_object_uses_verified_save_and_keeps_pending_memory_readback() -> None:
    ...
    assert "EditorSceneManager.SaveOpenScenes" not in block
    assert "SetGameObjectActiveTool.HandleCommand" in block
    assert "ComponentCrudCore.ResolveGameObject(normalized)" in block
    assert "Undo.RecordObject(target.gameObject" in block
