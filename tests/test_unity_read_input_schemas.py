...
def test_read_schemas_keep_single_owner_and_exact_pre_extraction_values() -> None:
    ...
    assert hashlib.sha256(encoded).hexdigest() == "209328def649f1b92f705d8bf5719b93ca18e9b5ba3cbf0fb8eaa64e0738af83"
...
def test_read_schema_owner_has_only_forward_dependencies() -> None:
    ...
    assert {node.module for node in ast.walk(tree) if isinstance(node, ast.ImportFrom)} == {
        "__future__", "typing", "external_mcp_tool_blocks", "path_to_skill_controller", "unity_shared_input_schemas",
    }
