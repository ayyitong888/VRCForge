...
for (const key of ["aboutTitle", "aboutProduct", "aboutMcp", "aboutSafety", "aboutLicense"]) {
  assert.match(workspace, new RegExp(`t\\("settings\\.${key}"(?:[,\\)])`), `About must render settings.${key}`);
}
...
assert.ok(!panel.includes('t("settings.logRedaction")'));
assert.ok(!panel.includes('t("settings.logRetention")'));
assert.ok(!panel.includes("retentionDays"));
...
for (const [name, entries] of flattened) {
  ...
  assert.equal(entries.get("settings.aboutProduct") || "", "VRCForge {{version}}");
  assert.match(entries.get("settings.aboutMcp") || "", /MCP 2\.0.*2026-07-28.*95/s);
