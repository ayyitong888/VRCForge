...
class AgentSkillRegistryService:
    ...
    def _normalize_user_skill(self, payload: dict[str, Any], existing_id: str | None = None) -> dict[str, Any]:
        ...
        return {
            ...
            "requiredResources": ensure_string_list(first_payload_value(payload, "requiredResources", "required_resources", "required-resources")) if first_payload_value(payload, "requiredResources", "required_resources", "required-resources") is not None else None,
            "gameOnlyAcceptance": ensure_string_list(first_payload_value(payload, "gameOnlyAcceptance", "game_only_acceptance", "game-only-acceptance")) if first_payload_value(payload, "gameOnlyAcceptance", "game_only_acceptance", "game-only-acceptance") is not None else None,
            "workflowDomain": str(first_payload_value(payload, "workflowDomain", "workflow_domain", "workflow-domain", default="")).strip()[:80],
