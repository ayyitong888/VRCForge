...
from unity_mcp_core_auth import (
    TRANSPORT_SCHEMA, MODERN_PROTOCOL_VERSION, MINIMUM_CORE_PROTOCOL_VERSION,
    UnityMcpCoreError, UnityMcpCoreConnectionError, UnityMcpCoreConnection,
    load_unity_mcp_core_connection, load_core_diagnostic_binding,
    authenticated_core_request, _resolve_project_root, _is_protocol_version,
)
...
def probe_unity_mcp_core_diagnostics(
    ...
    *,
    ...
    root, descriptor, port, token, instance_id, project_id = load_core_diagnostic_binding(project_root)
...
def _probe_core_request(
    ...
    payload = authenticated_core_request(token, request_id, method, params)
...
class UnityMcpCoreClient:
    ...
    def _send_modern(self, connection: socket.socket, request_id: int, method: str, params: dict[str, Any],
        ...
        self._write_line(connection, authenticated_core_request(
            self._connection.discovery_token, request_id, method, request_params,
        ))
