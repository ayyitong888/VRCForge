...
namespace VRCForge.Editor
{
    ...
    public static class RendererMaterialSlotTool
    {
        ...
        {
            if (Required(p, "expectedScenePath").Replace('\\', '/') != s.Path || Required(p, "expectedSceneGuid") != s.Guid || p["expectedSceneHandle"]?.Value<int>() != s.Handle || Required(p, "expectedRendererComponentType") != t.Identity.componentType || p["expectedRendererComponentIndex"]?.Value<int>() != t.Identity.componentIndex || Required(p, "rendererComponentId") != id || p["slotIndex"]?.Value<int>() != slot || Required(p, "expectedNewMaterialGuid") != next.Guid || Required(p, "expectedNewMaterialFileDigest") != next.Digest || before == null || Required(p, "expectedBeforeMaterialAssetPath") != before.Path || Required(p, "expectedBeforeMaterialGuid") != before.Guid || Required(p, "expectedBeforeMaterialFileDigest") != before.Digest || Required(p, "expectedBeforeMaterialShader") != (beforeMaterial.shader == null ? "" : beforeMaterial.shader.name) || p["expectedBeforeMaterialRenderQueue"]?.Value<int>() != beforeMaterial.renderQueue) throw new InvalidOperationException("The current state no longer matches the verified preview.");
        }
